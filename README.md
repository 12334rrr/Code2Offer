...
2. **覆盖矩阵强制出题**:由 `coverage.ts` 硬性分配 类别×难度×模块 配额(题量按模式自适应:经济 20 / 平衡 40 / 深度 60,`--questions` 可覆盖;配额矩阵等比缩放,11 类别广度不丢),分批生成,每题运行时校验:引用的文件必须真实存在、行号必须在范围内。
...
阶段2 出题 → questions.json                覆盖矩阵(经济20/平衡40/深度60,--questions 可覆盖),每题运行时校验引用
...
- `economy`:20 题、轻量校验，适合先看方向；报告会明确标注“不完整”，不能当作完整题库结果。
- `balanced`(默认):40 题、平衡批次和证据校验。
- `deep`:60 题、全部对抗校验与更严格预算；适合最终面试材料。
- 题量质量优先,不硬凑整数:配额矩阵等比缩放、11 类别广度不丢;`--questions 10~100` 可按仓库大小自行覆盖(小仓库少出题,省 token 且每题更聚焦)。
