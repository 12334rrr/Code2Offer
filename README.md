...
| `校验报告.md` / `自评报告.md` | 反幻觉校验结果 / DeepSeek 评委打分(满分 10) |
...
2. **覆盖矩阵强制出题**:一次要 100 题必然得到 30 题真话 + 70 题换皮。这里由 `coverage.ts` 硬性分配 类别×难度×模块 配额(合计恒 100),分批生成,每题运行时校验:引用的文件必须真实存在、行号必须在范围内。
...
另外:按内容哈希**缓存 + 阶段门控**(重跑只花增量钱)、**JD 加权重排**(必考 Top20 + STAR 定制)、**模拟排练**(评分+追问+弱项追踪)、**自评环**(DeepSeek 当评委打分到 10 分)。
...
node dist/cli/index.js generate ./你的仓库 --jd ./jd.txt

# 3) 使用
#    - 浏览器打开 <仓库>/interview-output/index.html(搜索/筛选/隐藏答案自测)
#    - 模拟排练:逐题作答 → 评分 → 追问 → 弱项记录
node dist/cli/index.js rehearse ./你的仓库/interview-output --count 5 --top20

#    - 自评环:DeepSeek 当评委,给材料打分(满分 10)+ 改进清单
node dist/cli/index.js evaluate ./你的仓库/interview-output
```
...
阶段2 出题 → questions.json                覆盖矩阵 100 题,每题运行时校验引用
...
阶段5 总装 → 01~06.md + index.html         叙述类 LLM 生成,题库确定性渲染
      └─ evaluate → 自评报告.md            DeepSeek 评委按维度打分
阶段6 排练(rehearse 命令)→ 排练记录.md      评分/追问/弱项追踪
```

**成本控制**:每个阶段产物落盘;`state.json` 记录输入哈希,未变化阶段直接跳过;LLM 调用按内容哈希缓存。改 JD 只重跑阶段 4-5;改仓库代码只重跑受影响阶段。

## 模型与密钥(.env)

- `DEEPSEEK_MODEL` 支持官方 `deepseek-chat` / `deepseek-reasoner`,也支持推理型模型名(如 `deepseek-flash`):客户端检测到推理型行为(空响应/思考耗尽预算)会**自动扩大 max_tokens 重试**;模型名不存在时自动回退 `deepseek-chat` 并在日志标注。
...
```
src/core/      config / deepseek客户端 / profiler画像 / chunker / coverage矩阵 / prompts / schemas / runner编排
