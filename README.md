...
2. **覆盖矩阵强制出题**:由 `coverage.ts` 硬性分配 类别×难度×模块 配额(题量按模式自适应:经济 30 / 平衡 60 / 深度 80,`--questions` 可覆盖;配额矩阵等比缩放,11 类别广度不丢),分批生成,每题运行时校验:引用的文件必须真实存在、行号必须在范围内。
...
阶段2 出题 → questions.json                覆盖矩阵(经济30/平衡60/深度80,--questions 可覆盖),每题运行时校验引用
...
- `economy`:30 题、轻量校验，适合先看方向；报告会明确标注“不完整”，不能当作完整题库结果。
- `balanced`(默认):60 题、平衡批次和证据校验。
- `deep`:80 题、全部对抗校验与更严格预算；适合最终面试材料。
